    ...
    <div class=" sm:px-8">
            ...
            <div class="bg-white rounded-xl shadow overflow-hidden">
                <div class="p-4 border-b border-gray-200 space-y-3">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        ...
                        <button id="refreshBatchesBtn"
                            class="w-full sm:w-auto min-h-[44px] px-3 py-1.5 bg-gray-100 rounded-lg text-sm hover:bg-gray-200 cursor-pointer touch-manipulation">Refresh</button>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-2">
                        <input id="batchSearch" type="text" class="border border-gray-300 rounded-lg p-2 text-sm"
                        ...
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                            <button id="applyBatchFiltersBtn"
                                class="w-full sm:w-auto min-h-[44px] px-3 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 cursor-pointer touch-manipulation">Apply</button>
                            <button id="resetBatchFiltersBtn"
                                class="w-full sm:w-auto min-h-[44px] px-3 py-2 bg-gray-100 rounded-lg text-sm hover:bg-gray-200 cursor-pointer touch-manipulation">Reset</button>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="w-full min-w-[780px]">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-sm">Uploaded By</th>
                ...
                <div
                    class="p-4 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="w-full sm:w-auto flex items-center justify-between sm:justify-start gap-2">
                        <button id="batchPrevPage"
                            class="min-h-[44px] px-3 py-1.5 bg-gray-100 rounded-lg text-sm hover:bg-gray-200 disabled:opacity-50 cursor-pointer touch-manipulation">Previous</button>
                        <span id="batchPageLabel" class="text-sm text-gray-600">Page 1 / 1</span>
                        <button id="batchNextPage"
                            class="min-h-[44px] px-3 py-1.5 bg-gray-100 rounded-lg text-sm hover:bg-gray-200 disabled:opacity-50 cursor-pointer touch-manipulation">Next</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($isAdmin)
            <div id="permissionsPanel" class="tab-panel hidden">
                <div class="bg-white rounded-xl shadow overflow-hidden">
                    <div
                        class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div id="rowsModal" class="fixed inset-0 bg-black bg-opacity-40 hidden z-40 items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-6xl max-h-[90vh] sm:max-h-[92vh] flex flex-col">
                <div
                    class="p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <h3 id="rowsTitle" class="font-semibold text-lg text-gray-900">Inventory Rows</h3>
                    <button id="closeRowsModal"
                        class="w-full sm:w-auto min-h-[44px] text-gray-500 hover:text-gray-700 cursor-pointer touch-manipulation">Close</button>
                </div>
                ...
                <div class="p-4 border-b border-gray-200">
                    <form id="rowForm" class="grid grid-cols-1 md:grid-cols-7 gap-2">
                        ...
                        <input id="rowWidth" type="number" step="0.01" min="0" class="border rounded p-2"
                            placeholder="Width">
                        <input id="rowHeight" type="number" step="0.01" min="0" class="border rounded p-2"
                            placeholder="Height">
                        ...
                        <div class="flex flex-col sm:flex-row gap-2">
                            <button type="submit"
                                class="w-full sm:w-auto min-h-[44px] bg-blue-600 text-white rounded px-3 py-2 text-sm cursor-pointer touch-manipulation">Save
                                Row</button>
                            <button type="button" id="resetRowForm"
                                class="w-full sm:w-auto min-h-[44px] bg-gray-100 rounded px-3 py-2 text-sm cursor-pointer touch-manipulation">Reset</button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        async function loadBatches(page) {
            try {
                ...
                const rows = result.data || [];
                console.log(rows);
                console.log('loadBatches result:', result);
                const pagination = result.pagination || {};
                batchPaginationState = {
                    total: pagination.total || 0,
                    current_page: pagination.current_page || 1,
                ...
                body.innerHTML = rows.map((batch, index) => {
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3">${batch.uploaded_by?.name || '-'}</td>
                        <td class="px-4 py-3 space-x-2">
