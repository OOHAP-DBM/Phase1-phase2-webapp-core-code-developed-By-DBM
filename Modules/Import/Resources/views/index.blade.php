...
@section('content')
    <div class="p-4 sm:p-6 lg:p-8">
        ...
        <div id="uploadTimerWidget" class="fixed bottom-4 left-3 right-3 sm:left-auto sm:right-4 z-50 hidden">
            <div id="uploadTimerExpanded"
                class="bg-white border border-blue-200 rounded-xl shadow-lg p-4 w-full sm:w-80 max-w-sm">
                <div class="flex items-start justify-between gap-2">
                    <div>
                        ...
                        <p id="uploadTimerMessage" class="text-xs text-blue-700 mt-1">
                            Currently System is mapping your inventory. Estimated processing time: 1–2 minutes. You can
                            continue browsing other screens.
                        </p>
                    </div>
                    <div class="flex items-center gap-1">
                        <button id="minimizeUploadTimer" type="button"
                            class="px-2 py-1 text-xs bg-blue-50 text-blue-700 rounded hover:bg-blue-100">_</button>
                    </div>
                </div>
                <div class="mt-3">
            ...
            <button id="uploadTimerMinimized" type="button"
                class="hidden bg-white border border-blue-200 rounded-full shadow px-3 py-2 text-xs font-semibold text-blue-800 hover:bg-blue-50">
                Processing 05:00
            </button>
        </div>
        ...
        <div class="flex flex-wrap gap-2 mb-6">
            ...
            <button onclick="openImportManagement('processing')"
                ...
                Processing
                <span id="processingBatches"
                    class="bg-orange-200 text-orange-800 text-xs font-bold px-1.5 py-0.5 rounded-full">0</span>
            </button>
            ...
            <button onclick="openImportManagement('approved')"
                ...
                Completed
                <span id="approvedBatches"
                    class="bg-green-200 text-green-800 text-xs font-bold px-1.5 py-0.5 rounded-full">0</span>
            </button>
            ...
            <button onclick="openImportManagement('completed')"
                ...
                <span id="completedBatches"
                    class="bg-gray-200 text-gray-800 text-xs font-bold px-1.5 py-0.5 rounded-full">0</span>
            </button>
            ...
            <button onclick="openImportManagement('failed')"
                ...
                Failed
                <span id="failedBatches"
                    class="bg-red-200 text-red-800 text-xs font-bold px-1.5 py-0.5 rounded-full">0</span>
            </button>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sm:p-7 max-w-7xl">
            ...
            <div id="vendorModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-[9999]">

                <div class="bg-white rounded-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto p-6 relative">

                    <button type="button" id="closeVendorModal" class="absolute top-4 right-4 text-2xl">
                        ×
                    </button>
            ...
            <form id="uploadForm" class="space-y-5" autocomplete="off">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-6">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="media_type" value="ooh" class="w-4 h-4 accent-green-600"
                                onchange="onHoardingTypeChange(this.value)">
                            <span class="text-sm text-gray-700">OOH (Out of Home)</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="media_type" value="dooh" class="w-4 h-4 accent-green-600"
                                onchange="onHoardingTypeChange(this.value)">
                ...
                @if($isAdmin ?? false)
                    <div id="vendorSelectionWrap" class="mb-5">
                        ...
                        <div class="flex gap-2">
                            <select id="vendor_id" name="vendor_id"
                                class="w-full rounded-lg border border-gray-300 px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            ...
                            <button type="button" id="createVendorBtn"
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            Leave empty to import under Admin account.
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <label for="excelFile" id="excelDropZone"
                            ...
                            <p id="excelFileName"
                                class="mt-1.5 text-xs text-green-700 font-medium hidden flex items-center gap-1">
                                <span
                                    class="w-5 h-5 flex items-center justify-center rounded-full bg-green-100 text-green-700 mr-1">
                        ...
                        <!-- PowerPoint -->
                        <div>
                            <p class="text-xs font-semibold text-gray-600 mb-2">PowerPoint File / PDf</p>
                            <div class="relative">
                                <input type="file" id="pptFile" name="ppt_file" accept=".pptx,.ppt,.pdf" class="hidden" />
                                <label for="pptFile" id="pptDropZone"
                                    class="flex flex-col items-center justify-center w-full h-44 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-all duration-200 group">
                                    <svg class="w-8 h-8 text-blue-400 mb-2" fill="none" stroke="currentColor"
                                        stroke-width="1.5" viewBox="0 0 24 24">
                                        <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="none" />
                ...
                <div id="submitWrap" class="hidden">
                    <button type="submit" id="submitBtn"
                        ...
                        <svg id="submitIcon" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-semibold text-blue-900 flex items-center gap-2">
                                <svg class="w-4 h-4 animate-spin text-blue-600" fill="none" stroke="currentColor"
                        ...
                        <div class="w-full bg-blue-100 rounded-full h-3 overflow-hidden">
                            <div id="uploadProgressBar" class="bg-blue-600 h-3 rounded-full transition-all duration-300"
                                style="width: 0%">
                            </div>
                        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden hidden mt-8">
            ...
            <div
                class="px-4 py-4 sm:px-6 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div class="w-full sm:w-auto flex items-center justify-between sm:justify-start space-x-2">
                    <button id="historyPrevBtn"
                        class="min-h-[44px] px-3 py-1.5 bg-gray-100 rounded-lg text-sm hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed">Previous</button>
                    <span id="historyPageLabel" class="text-sm text-gray-600">Page 1 / 1</span>
                    <button id="historyNextBtn"
                        class="min-h-[44px] px-3 py-1.5 bg-gray-100 rounded-lg text-sm hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
                </div>
            </div>
        </div>
        ...
        <div id="approveModal" class="fixed inset-0  bg-opacity-50 z-40 hidden flex items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-2xl max-w-md w-full">
                ...
                <div class="p-6">
                    <p class="text-gray-600 mb-4">Are you sure you want to approve this inventory? This will create all
                        hoardings from valid records.</p>
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                        <p class="text-sm text-blue-800"><strong>Invendtory ID:</strong> <span id="approveBatchId"
                                class="font-mono">N/A</span></p>
                        <p class="text-sm text-blue-800 mt-2"><strong>Valid Records:</strong> <span
                                id="approveBatchValid">0</span></p>
                    </div>
                </div>
                <div class="px-6 py-4 border-t border-gray-200 flex flex-col-reverse sm:flex-row justify-end gap-3">
                    <button onclick="closeApproveModal()"
                        class="w-full sm:w-auto min-h-[44px] px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium">Cancel</button>
                    <button onclick="confirmApprove()" id="confirmApproveBtn"
                        class="w-full sm:w-auto min-h-[44px] px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium disabled:opacity-50">Approve</button>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col max-h-screen">
                ...
                <div class="px-6 py-4 border-t border-gray-200 flex justify-end">
                    <button onclick="closeErrorModal()"
                        class="px-4 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium">Close</button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        const UPLOAD_LOCK_DURATION_SECONDS = 120;
                ...
                toast.innerHTML = `
                                                                        <div class="flex items-start gap-2">
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <p class="text-xs mt-0.5 text-orange-100">Your import is taking longer than expected. Please check Import History.. The file may still be processing in the background.</p>
                                                                            </div>
                                                                        </div>
        ...
        async function submitUpload(e) {
            ...
            try {
                ...
                if (uploadedBatchId) {
                    startUploadLock(uploadedBatchId);
                    showToast('Please wait. Processing started and upload is locked for up to 2 minutes.', 'info');
                }
                await loadBatches();
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const openBtn = document.getElementById('createVendorBtn');
            const modal = document.getElementById('vendorModal');

            // Open Vendor Modal
            openBtn.addEventListener('click', function () {

                fetch("{{ route('admin.vendors.create-modal') }}")
                    .then(res => res.text())
                    .then(html => {

                        document.getElementById('vendorFormContainer').innerHTML = html;

                        modal.classList.remove('hidden');
                        modal.classList.add('flex');

                        // Close Button
                        document.getElementById('closeVendorModal').onclick = function () {
                                ...
                                .then(data => {
                                    ...
                                    if (data.success) {
                                        ...
                                        const option = new Option(
                                            data.vendor.company_name ?? data.vendor.name,
                                            data.vendor.id,
                                            true,
                                            true
                                        );
            ...
            document.addEventListener('click', function (e) {
                ...
                if (e.target.id === 'closeVendorModal') {
                    modal.classList.add('hidden');
                    modal.classList.remove('flex');
                }
