...
<aside class="hidden lg:block w-64 bg-white border-r border-gray-200 overflow-y-auto" id="sidebar">
    <button id="mobile-btn-close" class="block md:hidden p-2 text-gray-600 hover:bg-gray-100 rounded-md text-end">
        ✕
    </button>
    ...
    <div class="bg-white px-6 pb-5">
        ...
        <div class="flex items-center space-x-3">
            <div style="width:56px; height:56px; min-width:56px; min-height:56px; max-width:56px; max-height:56px; border-radius:50%;
                overflow:hidden;border:1px solid #d1d5db; flex-shrink:0;display:inline-block;">
                @if(auth()->user()->avatar)
                            <img src="{{ str_starts_with(auth()->user()->avatar, 'http')
                    ? auth()->user()->avatar
                    : asset('storage/' . ltrim(auth()->user()->avatar, '/')) }}?v={{ auth()->user()->updated_at->timestamp }}"
                                alt="Profile Image" class="w-full h-full object-cover block">
                @else
                    {{-- Default User Icon --}}
    ...
    <nav class="px-4 pb-4">
        <div class="space-y-1">
            ...
            {{-- My Profile Dropdown --}}
            <div x-data="{ open: {{ request()->routeIs('customer.profile*') ? 'true' : 'false' }} }" class="space-y-1">
                {{-- Parent --}}
                ...
                <div x-show="open" x-collapse x-cloak class="space-y-1 pl-3 mt-1">
                    <a href="{{ route('customer.profile.index') }}" class="block px-6 py-1 text-sm rounded-md transition
    ...
    : 'text-gray-600 hover:bg-gray-50 hover:pl-5 border-transparent' }}">
                    ...
                    <a href="{{ route('customer.profile.billing') }}" class="block px-6 py-1 text-sm rounded-md transition
    ...
    : 'text-gray-600 hover:bg-gray-50 hover:pl-5 border-transparent' }}">
            ...
            {{-- Enquiries --}}
            {{-- Enquiries Dropdown --}}
            <div x-data="{ open: {{ request()->routeIs('customer.enquiries.*') || request()->routeIs('customer.direct.enquiries.*') ? 'true' : 'false' }} }"
                class="space-y-1">
                {{-- Enquiries Parent --}}
                <button type="button" @click="open = !open"
                    class="cursor-pointer w-full flex items-center justify-between gap-3 px-3 py-2 text-sm font-medium rounded-lg text-gray-700 hover:bg-gray-50">
                    <div class="flex items-center gap-3">

                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path
                                d="M17.275 20.25L20.75 16.8L19.7 15.75L17.275 18.125L16.3 17.15L15.25 18.225L17.275 20.25ZM6 9H18V7H6V9ZM18 23C16.6167 23 15.4377 22.5123 14.463 21.537C13.4883 20.5617 13.0007 19.3827 13 18C13 16.6167 13.4877 15.4377 14.463 14.463C15.4383 13.4883 16.6173 13.0007 18 13C19.3833 13 20.5627 13.4877 21.538 14.463C22.5133 15.4383 23.0007 16.617 23 18C23 19.3833 22.5123 20.5617 21.537 21.537C20.5617 22.5123 19.3827 23 18 23ZM3 22V5C3 4.45 3.196 3.97933 3.588 3.588C3.98 3.19667 4.45067 3.00067 5 3H19C19.55 3 20.021 3.196 20.413 3.588C20.805 3.98 21.0007 4.45067 21 5V11.675C20.6833 11.525 20.3583 11.4 20.025 11.3C19.6917 11.2 19.35 11.125 19 11.075V5H5V19.05H11.075C11.1583 19.5667 11.2877 20.0583 11.463 20.525C11.6383 20.9917 11.867 21.433 12.15 21.85L12 22L10.5 20.5L9 22L7.5 20.5L6 22L4.5 20.5L3 22Z"
                                fill="currentColor" />
                        </svg>
    ...
    : 'text-gray-600 hover:bg-gray-50' }}">
            ...
            <!-- Offers -->
            <div x-data="{ open: {{ request()->routeIs('customer.offers.*') ? 'true' : 'false' }} }" class="space-y-1">
        ...
        {{ request()->routeIs('customer.offers.*') ? 'bg-green-600 text-white' : 'text-gray-700 hover:bg-gray-50' }}">
                ...
                <div x-show="open" x-collapse x-cloak class="space-y-1 pl-3 mt-1">
                    <a href="{{ route('customer.offers.index') }}" class="block px-6 py-1 text-sm rounded-md transition
