...
<aside
    ...
    onclick="expandSidebarOnClick(event)"
>
    ...
    >
        <svg id="sidebar-arrow-icon" class="w-4 h-4 text-gray-600 transition-transform duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="py-2 px-2 rounded-lg {{ request()->routeIs('vendor.profile.*') ? 'bg-[#00995c] text-white' : '' }}">
            <a href="{{ route('vendor.profile.edit') }}" class="flex items-center space-x-3">
                ...
                >
                    @if(auth()->user()->avatar)
                        <img
                            src="{{ route('view-avatar', auth()->user()->id) }}?v={{ auth()->user()->updated_at->timestamp }}"
                            alt="Profile Image"
    ...
    <nav class="px-4 pb-4 flex-1 overflow-y-auto">
        <div class="space-y-1">
            ...
            {{-- Dashboard --}}
            <a href="{{ route('vendor.dashboard') }}" class="flex items-center px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('vendor.dashboard') ? 'bg-[#00995c] text-white' : 'text-gray-700 hover:bg-gray-50' }}">
            ...
            >
                ...
                >
                    <div class="flex items-center gap-3 {{ request()->routeIs('customer.enquiries.*') ? 'text-white' : 'text-gray-700' }}">
                        <span class="sidebar-icon flex items-center justify-center min-w-[24px]">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div
                x-data="{ open: @if(request()->routeIs('vendor.enquiries.*') || request()->routeIs('vendor.direct-enquiries.*')) true @else false @endif }"
                class="space-y-1"
            >
                <button
                    type="button"
                ...
                >
                    <div class="flex items-center gap-3 {{ (request()->routeIs('vendor.enquiries.*') || request()->routeIs('vendor.direct-enquiries.*')) ? 'text-white' : 'text-gray-700' }}">
                        <span class="sidebar-icon flex items-center justify-center min-w-[24px]">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z" fill="currentColor"/>
                            </svg>
