...
<tr class="border-b hover:bg-gray-50">
    ...
    <td class="px-3 py-3">
        <a href="{{ route('vendor.enquiries.show', $offer->enquiry->id) }}" class="font-bold text-gray-800 hover:underline">#{{ $offer->enquiry->id }}</a>
...
@endforelse --}}
{{-- resources/views/vendor/offers/partials/table.blade.php --}}
@forelse($offers as $index => $offer)
@php
    $hoardingCount = $offer->hoardingCount();
    $locationCount = $offer->locationCount();
    ...
    // FIX: these flags used to be computed independent of offer status, so an
    // offer that had a pending customer modification when it was accepted/
    // rejected kept showing "Modify Offer" / "Accept Customer's Offer"
    // forever. Gate everything behind isNegotiable() first.
    $negotiable = $offer->isNegotiable();
    $hasModRequest = $negotiable && $offer->hasPendingModificationRequest();
    $customerModified = $negotiable && $offer->status === 'sent' && $offer->wasLastModifiedByCustomer();

    $statusMap = [
        'draft'    => ['label' => 'Draft', 'class' => 'text-gray-500'],
        'sent'     => ['label' => 'Offer Sent', 'sub' => 'Waiting for Customer Response', 'class' => 'text-blue-600'],
        'accepted' => ['label' => 'Offer Accepted', 'class' => 'text-emerald-600'],
        'rejected' => ['label' => 'Offer Rejected', 'class' => 'text-red-600'],
        'expired'  => ['label' => 'Offer Expired', 'class' => 'text-orange-500'],
    ];
    $statusInfo = $statusMap[$offer->status] ?? ['label' => ucfirst($offer->status), 'class' => 'text-gray-500'];
    ...
    if ($negotiable) {
        ...
        // Accepted offers still get a next-step action, just never Modify/Reject
        $action = ['label' => 'Create Quotation', 'class' => 'bg-emerald-600 hover:bg-emerald-700'];
    } elseif ($offer->status === 'rejected' || $offer->status === 'expired') {
               $action = ['label' => 'Rejected', 'class' => 'bg-red-600 hover:bg-red-700'];

    }
    // status === 'rejected' or 'expired' → no action button at all (was
    // previously offering "Send Counter Offer", which routed straight back
    // into the modify page — removed, since a rejected offer should not be
    // modifiable; the vendor creates a fresh offer instead if they want to
...
<tr class="border-b hover:bg-gray-50">
    ...
    <td class="px-3 py-3 text-right">
        <button type="button" class="row-menu-trigger text-gray-400 hover:text-gray-700 px-2"
            data-offer-id="{{ $offer->id }}"
            data-view-url="{{ route('vendor.offers.show', $offer->id) }}"
            data-modify-url="{{ $negotiable ? route('vendor.offers.create', ['offer_id' => $offer->id]) : '' }}"
            data-negotiable="{{ $negotiable ? 'true' : 'false' }}">⋮</button>
    </td>
</tr>
@empty
<tr><td colspan="9" class="px-4 py-10 text-center text-gray-400 text-xs">No offers found</td></tr>
