// public/js/customer-offer-builder.js

const selected = new Map();
let hoardingsCache = [];
let currentPage = 1, totalPages = 1;
let heatmapMap = {}, currentEditId = null, dpFp = null, dpCurrentStart = null;
let isSubmittingModification = false;
...
document.getElementById('offer-confirm-yes-btn').addEventListener('click', async () => {
    if (isSubmittingModification) return;
...
function rowOOH(i) {
    ...
    return `<tr><td class="px-3 py-3"><p class="font-bold text-gray-800">${i.title}</p><p class="text-[10px] text-gray-400">${i.city||''}</p></td>
...
function openPreview() {
    ...
    document.getElementById('op-vendor-name').innerText = window.OFFER_VENDOR?.name || '—';
...
async function confirmAndSend() {
    ...
    try {
        ...
        if (!res.success) {
            ...
            alert(res.message || 'Failed to send changes.');
            return;
        }

        document.getElementById('success-view-offer-btn').href = res.data.redirect;
        document.getElementById('offerSuccessModal').classList.remove('hidden');
    } catch (e) {
        console.error(e);
        alert('Something went wrong. Please try again.');
    } finally {
        btn.disabled = false; btn.innerText = 'Confirm & Send to Vendor';
    }
}
